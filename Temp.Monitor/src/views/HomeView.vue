<template>
  <div class="justify">
    ...
    ></apexchart>
    <div class="center">
      <button class="button" @click="getLog()">
      Get Data
    </button>
    <br>
    <Transition>
...
async function getLog(){ 
  ...
  if(records){
    ...
    records.forEach(record => {
        if(record.firstDay == (day-7).toString().padStart(2,'0')+ '/'+month && record.lastDay == day.toString().padStart(2,'0')+ '/'+day - 7<0? month:month1){
          ...
          for (let f=0;f<i;f++){
            ...
            setTimeout(()=>{
            store.dispatch('getData',{mode:3, deviceId:deviceId, startTime:getNoonTime() - (hours * 60 * 60 * 1000 + 25 * 60 * 1000), endTime:getNoonTime() - (hours * 60 * 60 * 1000)})
          },timer)
        }}
    });
  }
    ...
    for (let i=0;i<7;i++){
      ...
      setTimeout(()=>{
        if(i==0){
          store.dispatch('getData',{mode:3, deviceId:deviceId, startTime:getNoonTime() -  ( 25 * 60 * 1000), endTime:getNoonTime()})
        }
        else{
          store.dispatch('getData',{mode:3, deviceId:deviceId, startTime:getNoonTime() - (hours * 60 * 60 * 1000 + 25 * 60 * 1000), endTime:getNoonTime() - (hours * 60 * 60 * 1000)})
        }
