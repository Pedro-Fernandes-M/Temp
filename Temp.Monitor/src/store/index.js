...
const store = createStore({
      ...
      actions: {
        async getData(state,payload) {
          ...
          function setData(data){
            ...
            let dataFormatada = dia <10 ? `${dia.toString().padStart(2, '0')}`+'/'+month:`${dia+'/'+month}`;
          ...
          function stringToSign(query, method){
            ...
            if(query){
                ...
                if(payload.mode===1)
                  url = `/v1.0/token?${queryParams.toString()}`;
          ...
          function makeApiRequest() {
            ...
            const sign = payload.mode===1||payload.mode===2 ? calcSign(clientId, timestamp, nonce, signStr, secret) : calcSignB(clientId, state.getters.getAccessToken, timestamp, nonce, signStr, secret);
            ...
            axios.get(import.meta.env.VITE_API_HOST + url, { headers })
                .then(response => {
                    if(payload.mode==1||payload.mode==2 ){
        ...
        async createPDF(state) { 
          ...
          const month = page1!=null ? (currentDate.getMonth() + 1).toString().padStart(2, '0') : (currentDate.getMonth() ).toString().padStart(2, '0') ;
      ...
      let columnStartX = tableXStart + columnWidths.slice(0, columnIndex).reduce((a, b) => a + b, 0);
      ...
      page.drawText(header, {
          ...
          for (let i = 0; i < 31; i++) {
              ...
              const logForDay = logs.find(log => (log.day.split('/')[0] == ((i + 1).toString().padStart(2, '0')) && month == log.day.split('/')[1]));
          ...
          const [firstDay, lastDay] = [store.getters.getLogs[0]?.day, store.getters.getLogs[store.getters.getLogs.length - 1]?.day];
          if(records){
